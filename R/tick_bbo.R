...
tick_bbo <- function(mbp_order_book, trade_summary, trading.tz="America/Chicago",
  ...
  trade_summary <- trade_summary[Time %between% c(start_timestamp, end_timestamp)][, list(Time, Code, trd_price=PX, trd_size=Size, trd_agg=agg)]

  mbp_order_book <- mbp_order_book[Time %between% c(start_timestamp, end_timestamp)][, c("Time", "Code", "Bid_PX_1", "Ask_PX_1", "Bid_Qty_1", "Ask_Qty_1")]
