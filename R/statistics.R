...
if(length(OPEN_INT)!=0){
  open_int <- str_match_all(OPEN_INT, "83=([^,]*),107=([^,]*),269=C,271=([^,]*),")
...
if(length(LIMIT)!=0){
  ...
  LIMIT <- str_replace_all(LIMIT, "1128=([^,]*),9=([^,]*),35=([^,]*),49=([^,]*),34=([^,]*),","")
...
if(length(SIMULATE_SELL)!=0){
  ...
  simulate_sell <- str_match_all(SIMULATE_SELL, "83=([^,]*),107=([^,]*),269=E,270=([^,]*),271=([^,]*)")
...
if(length(SIMULATE_BUY)!=0){
  ...
  simulate_buy <- str_match_all(SIMULATE_BUY, "83=([^,]*),107=([^,]*),269=F,270=([^,]*),271=([^,]*)")
...
info_list <- list(open = open, settle = settle, high_px = high_px,
                  low_px = low_px, high_bid = high_bid, low_offer = low_offer,
                  volume = volume, open_int = open_int, simulated_buy = simulate_buy,
                  simulated_sell = simulate_sell, limit = limit)
  ...
  if(length(ELEC_VOLUME)!=0){
    ...
    elec_volume.info <- str_match_all(elec_volume.info, "75=([^,]*),52=([^,]*),60=([^,]*),")
  ...
  info_list <- list(open = open, settle = settle, high_px = high_px,
                    low_px = low_px, high_bid = high_bid, low_offer = low_offer,
                    volume = volume, open_int = open_int, elec_volume = elec_volume,
                    limit = limit)
  ...
  file <-  fread(input, header = F, sep="\\", fill=TRUE)[[1L]]
