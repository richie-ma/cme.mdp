...
mbo_quote_queue <- function(mbo_input,date,
  ...
  if (length(part1) != 0) {
    ...
    part1_order <- str_subset(
      part1,
      "279=[012],269=([01]*),55=([^,]*),270=([^,]*),37706=([^,]*),37707=([^,]*),37=([^,]*),"
    )
    info_part1 <- str_extract_all(part1_order,
                                  "75=([^,]*),34=([^,]*),52=([^,]*),60=([^,]*),")
    order_part1 <- str_match_all(
      part1_order,
      "279=([012]*),269=([01]*),55=([^,]*),270=([^,]*),37706=([^,]*),37707=([^,]*),37=([^,]*),"
    )
    n_row_1 <- sapply(order_part1, nrow)
    info_part1 <- str_dup(info_part1, n_row_1)
    info_part1 <- str_match_all(info_part1,
                                "75=([^,]*),34=([^,]*),52=([^,]*),60=([^,]*),")
  ...
  if (isTRUE(implied_quotes)) {
    ...
    if (length(part1.1) != 0) {
      ## extract MBP info
      ## we need to include the implied orders here.
      part1.1_order <- str_subset(
        part1.1,
        "279=[012],269=([EF]*),55=([^,]*),83=([^,]*),270=([^,]*),271=([^,]*),"
      )
      info_part1.1 <- str_extract_all(part1.1_order,
                                      "75=([^,]*),34=([^,]*),52=([^,]*),60=([^,]*),")
      order_part1.1 <- str_match_all(
        part1.1_order,
        "279=([012]*),269=([EF]*),55=([^,]*),83=([^,]*),270=([^,]*),271=([^,]*),"
      )
      n_row_1.1 <- sapply(order_part1.1, nrow)
      info_part1.1 <- str_dup(info_part1.1, n_row_1.1)
      info_part1.1 <- str_match_all(info_part1.1,
                                    "75=([^,]*),34=([^,]*),52=([^,]*),60=([^,]*),")
  ...
  if (length(part2) != 0) {
    order_part2 <- str_subset(part2,
                              "37=([^,]*),37707=([^,]*),37706=([^,]*),9633=([^,]*),37708=([^,]*),")

    order_part2.order <- str_match_all(
      order_part2,
      "37=([^,]*),37707=([^,]*),37706=([^,]*),9633=([^,]*),37708=([^,]*),"
    )
    ...
    info_part2.time <- str_match_all(info_part2.time,
                                     "75=([^,]*),34=([^,]*),52=([^,]*),60=([^,]*),")
    ...
    setnames(
      info_part2.time,
      c("V2", "V3", "V4", "V5"),
      c("Date", "MsgSeq", "SendingTime", "TransactTime")
    )
