...
status <- function(input, date){
  ...
  data <-  fread(input, header = F, sep="\\", fill=TRUE)[[1L]]
  ...
  if(date < "2015-11-20"){
    ...
    if(length(Index1)!=0){
      ...
      Index1 <- str_match_all(Index1, "34=([^,]*),52=([^,]*),60=([^,]*),75=([^,]*),326=([^,]*),1174=([^,]*),")



      Index1 <- as.data.table(do.call(rbind, Index1))[,-1]

      names(Index1)[c(1:6)] <- c("MsgSeq","SendingTime","TransactTime","Date","TradingStatus", "TradingEvent")
    ...
    if(length(Index2)!=0){
      ...
      Index2 <- str_match_all(Index2, "34=([^,]*),52=([^,]*),60=([^,]*),75=([^,]*),326=([^,]*),1174=([^,]*),")
      Index2 <- as.data.table(do.call(rbind, Index2))[,-1]
      names(Index2)[c(1:6)] <- c("MsgSeq","SendingTime", "TransactTime","Date","TradingStatus", "TradingEvent")
    ...
    if(length(Index3)!=0){
      ...
      Index3 <- str_match_all(Index3, "34=([^,]*),52=([^,]*),60=([^,]*),75=([^,]*),326=([^,]*),1174=([^,]*),")
      Index3 <- as.data.table(do.call(rbind, Index3))[,-1]
      names(Index3)[c(1:6)] <- c("MsgSeq","SendingTime","TransactTime","Date","TradingStatus", "TradingEvent")
